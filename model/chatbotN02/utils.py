...
def maskNLLLoss(inp, target, mask):
    ...
    crossEntropy = -torch.log(
        torch.gather(inp, 1, target.view(-1, 1)).squeeze(1), device=DEVICE_TRAIN
    )
...
def train(
    ...
    # Set device options
    input_variable = input_variable.to(DEVICE_TRAIN)
    target_variable = target_variable.to(DEVICE_TRAIN)
    mask = mask.to(DEVICE_TRAIN)
    # Lengths for rnn packing should always be on the cpu
    lengths = lengths.to(DEVICE_INFERENCE)
    ...
    decoder_input = decoder_input.to(DEVICE_TRAIN)
    ...
    if use_teacher_forcing:
        ...
        for t in range(max_target_len):
            ...
            decoder_input = torch.LongTensor(
                [[topi[i][0] for i in range(batch_size)]], device=DEVICE_TRAIN
            )
            # Calculate and accumulate loss
            mask_loss, nTotal = maskNLLLoss(decoder_output, target_variable[t], mask[t])
            loss += mask_loss
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        if iteration % save_every == 0:
            ...
            torch.save(
                ...
                os.path.join(directory, "{}_{}.pth".format(iteration, "checkpoint")),
            )
...
class GreedySearchDecoder(nn.Module):
    def __init__(self, encoder, decoder):
        ...
        self.encoder = encoder.to(DEVICE_INFERENCE)
        self.decoder = decoder.to(DEVICE_INFERENCE)
    ...
    def forward(self, input_seq, input_length, max_length):
        ...
        decoder_input = (
            torch.ones(1, 1, device=DEVICE_INFERENCE, dtype=torch.long) * SOS_token
        )
...
if __name__ == "__main__":
    ...
    conversations = (
        []
    )  # list of dictionaries: "character1ID", "character2ID", "movieID", "utteranceIDs", "lines"
    # lines: list of dictionaries of lines
    ...
    if loadFilename:
        checkpoint = torch.load(loadFilename, map_location=DEVICE_TRAIN)
    ...
    encoder = encoder.to(DEVICE_TRAIN)
    decoder = decoder.to(DEVICE_TRAIN)
